...
vac v0.1.0 is the initial release of the Marp-to-Video conversion tool. It enables you to transform Marp markdown presentations into narrated videos with AI-generated voiceovers using ElevenLabs TTS.
...
- `vac tts` - Generate audio files from transcript.json
- `vac video` - Create video from Marp presentation with audio
...
```bash
go install github.com/grokify/videoascode/cmd/vac@v0.1.0
```
...
2. Generate audio: `vac tts -i presentation.md -o audio/`
3. Create video: `vac video -i presentation.md -m audio/manifest.json -o output.mp4`

See the [README](https://github.com/grokify/videoascode#readme) for detailed usage instructions.
